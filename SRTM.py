...
class SRTM:
    ...
    def write_reg(self, reg, write):
        ...
        try:
            # reg_name = "axi_boardinfo_user_reg" + str(reg)
            wri = self._ipbus.write(reg, write)
    ...
    def read_all_boards(self):
        ...
        print(f'efuse:      {hex(efuse)}')

        high = self.read_board("high")
        middle = self.read_board("middle")
        low = self.read_board("low")
        print(f'dna high:   {hex(high)}')
        print(f'dna middle: {hex(middle)}')
        print(f'dna low:    {hex(low)}')

        for i in range(self.num_registers):
            reg = self.read_board(i)
            print(f'userreg {i}:  {hex(reg)}')
        return
